...
const SuspendUser = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // The backend /api/suspendedUsers now automatically excludes soft-deleted items
  ...
  const handlePageChange = (newPage) => {
    if (newPage >= 1 && newPage <= totalPages) {
      navigate(`/suspended-users?page=${newPage}&limit=${itemsPerPage}`);
    }
  };
  ...
  const performSoftDelete = async () => {
    console.log("SuspendUser: performSoftDelete function called.");
    ...
    try {
      ...
      console.log("SuspendUser: Attempting to soft-delete report. URL:", deleteUrl);
      console.log("SuspendUser: Sending report _id:", userToDelete._id);
      console.log("SuspendUser: Sending Authorization header with token (first 10 chars):", idToken.substring(0, 10) + '...');
      ...
      console.log("SuspendUser: Soft-delete response status:", response.status);
      const responseText = await response.text();
      let result;
      try {
        result = JSON.parse(responseText);
        console.log("SuspendUser: Soft-delete response JSON:", result);
      } catch (jsonError) {
        console.error("SuspendUser: Soft-delete response was not JSON. Raw text:", responseText);
      ...
      console.error("SuspendUser: Soft deletion error:", err);
  ...
  const performPermanentDelete = async () => {
    console.log("SuspendUser: performPermanentDelete function called.");
    ...
    try {
      ...
      console.log("SuspendUser: Attempting to permanently delete report. URL:", deleteUrl);
      console.log("SuspendUser: Sending report _id:", userToDelete._id);
      console.log("SuspendUser: Sending Authorization header with token (first 10 chars):", idToken.substring(0, 10) + '...');
      ...
      console.log("SuspendUser: Permanent delete response status:", response.status);
      const responseText = await response.text();
      let result;
      try {
        result = JSON.parse(responseText);
        console.log("SuspendUser: Permanent delete response JSON:", result);
      } catch (jsonError) {
        console.error("SuspendUser: Permanent delete response was not JSON. Raw text:", responseText);
        throw new Error(`Server responded with non-JSON content (Status: ${response.status}). Raw: ${responseText.substring(0, 100)}...`);
      }
      ...
      console.error("SuspendUser: Permanent deletion error:", err);
  ...
  if (loading || authLoading) {
    return (
      <div className="container mx-auto p-4 text-center">
        <p className="text-xl text-gray-600">Loading suspended user data...</p>
    ...
    <div className="container mx-auto p-4">
      ...
      {suspendedUsers.length === 0 ? (
        <p className="text-center text-gray-700 text-lg">
          No suspended users found at the moment.
        </p>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {suspendedUsers.map((user) => (
              <div
                key={user._id}
                className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-yellow-300 transform hover:-translate-y-1 hover:scale-105"
              >
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="text-sm text-gray-600 mb-4 space-y-1">
                      <p className="flex items-center">
                        ...
                        <span className="ml-2 px-3 py-1 bg-yellow-600 text-white rounded-full text-xs font-semibold uppercase tracking-wide">
                          {user.status}
                        </span>
                      </p>
                      <p className="flex items-center">
                      ...
                      {user.reporterName && (
                        <p className="flex items-center mt-3">
                          ...
                          <span className="ml-2 text-yellow-700 font-bold flex-grow min-w-0">
                            {user.reporterName}
                          </span>
                        </p>
                      )}
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200">
                      <p className="font-semibold text-gray-800 mb-2">Reason:</p> {/* REASON FIELD */}
