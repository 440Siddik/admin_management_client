...
const BannedUser = () => {
  ...
  const { idToken, currentUser, loading: authLoading, userRole } = useAuth();

  // Use SERVER_URL directly
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const url = `${backendUrl}/api/bannedUsers?page=${pageFromUrl}&limit=${itemsPerPage}`;
      const response = await fetch(url);

      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: 'Unknown error or non-JSON response from server.' }));
        throw new Error(errorData.message || 'Failed to fetch banned users.');
      }
      ...
      setError(err.message || 'Could not load banned users. Check network and backend CORS configuration.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePageChange = (newPage) => {
    if (newPage >= 1 && newPage <= totalPages) {
      navigate(`/banned-users?page=${newPage}&limit=${itemsPerPage}`);
    }
  };
  ...
  const performSoftDelete = async () => {
    console.log("BannedUser: performSoftDelete function called.");
    ...
    try {
      ...
      console.log("BannedUser: Attempting to soft-delete report. URL:", deleteUrl);
      console.log("BannedUser: Sending report _id:", userToDelete._id);
      console.log("BannedUser: Sending Authorization header with token (first 10 chars):", idToken.substring(0, 10) + '...');
      ...
      console.log("BannedUser: Soft-delete response status:", response.status);
      const responseText = await response.text();
      let result;
      try {
        result = JSON.parse(responseText);
        console.log("BannedUser: Soft-delete response JSON:", result);
      } catch (jsonError) {
        console.error("BannedUser: Soft-delete response was not JSON. Raw text:", responseText);
      ...
      console.error("BannedUser: Soft deletion error:", err);
  ...
  const performPermanentDelete = async () => {
    console.log("BannedUser: performPermanentDelete function called.");
    ...
    try {
      ...
      console.log("BannedUser: Attempting to permanently delete report. URL:", deleteUrl);
      console.log("BannedUser: Sending report _id:", userToDelete._id);
      console.log("BannedUser: Sending Authorization header with token (first 10 chars):", idToken.substring(0, 10) + '...');
      ...
      console.log("BannedUser: Permanent delete response status:", response.status);
      const responseText = await response.text();
      let result;
      try {
        result = JSON.parse(responseText);
        console.log("BannedUser: Permanent delete response JSON:", result);
      } catch (jsonError) {
        console.error("BannedUser: Permanent delete response was not JSON. Raw text:", responseText);
        throw new Error(`Server responded with non-JSON content (Status: ${response.status}). Raw: ${responseText.substring(0, 100)}...`);
      }
      ...
      console.error("BannedUser: Permanent deletion error:", err);
  ...
  if (loading || authLoading) {
    return (
      <div className="container mx-auto p-4 text-center">
        <p className="text-xl text-gray-600">Loading banned user data...</p>
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {bannedUsers.length === 0 ? (
        <p className="text-center text-gray-700 text-lg">No banned users found at the moment.</p>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {bannedUsers.map((user) => (
              <div
                key={user._id}
                className="bg-gradient-to-br from-red-50 to-red-100 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-red-300 transform hover:-translate-y-1 hover:scale-105"
              >
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="text-sm text-gray-600 mb-4 space-y-1">
                      <p className="flex items-center">
                        ...
                        <span className="ml-2 px-3 py-1 bg-red-600 text-white rounded-full text-xs font-semibold uppercase tracking-wide">
                          {user.status}
                        </span>
                      </p>
                      <p className="flex items-center">
                      ...
                      {user.reporterName && (
                        <p className="flex items-center mt-3">
                          ...
                          <span className="ml-2 text-red-700 font-bold flex-grow min-w-0">
                            {user.reporterName}
                          </span>
                        </p>
                      )}
